...
승격이 **보류(hold)** 되는 경우에도 트랙은 살아서 히트·색 투표·관측 이력을 계속 쌓습니다 —
(a) 출발 구역 랜드마크 근처, seam이 아직 등록되지 않았을 때(드리프트된 포즈로 옛 지도를
새 콘으로 오인하는 구간, `csm_loop_window_m` 반경), (b) 오래 못 본 랜드마크 근처의 drift-twin 홀드.
seam이 등록되면 보류된 트랙이 쌓인 색과 이력을 그대로 들고 승격되고, 큰 보정(seam/최적화
≥0.5 m)이나 freeze 직전에 프런트엔드를 리셋할 때는 수렴한 트랙을 먼저 꺼내(`drainPromotable`)
보정된 키프레임 위에 다시 앵커링해 등록합니다. **매핑 중 `cone_map`에는 수렴한 보류 트랙도
미리 실려 나갑니다**(`promotablePreviews`) — 그래프 승인과 무관한 가시성 전용으로, 보류 구간에서
local planner가 콘 없는 맵을 보고 흔들리던 문제(2026-08-24 관찰) 때문입니다. (2026-08-23 이전에는 보류 시 트랙을 폐기해
재관측마다 복사본을 새로 만들었고, flush에서 가장 먼저 만든 — 원거리·무색 — 복사본만 살아남아
랩 1 마지막 구간이 비거나 무색으로 얼었습니다.)
...
기준(추가 5히트, 삭제는 20 m 연속 실명)으로만 콘 추가·삭제를 허용합니다 (오렌지 게이트 콘은 절대 삭제 불가).
삭제·트랙 kill 예산은 전부 **미터 기준**입니다 — 횟수 기준은 처리 주기를 바꾸면 의미가 몰래 바뀝니다(2026-08-24).
