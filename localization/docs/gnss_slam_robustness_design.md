# 복합 GNSS 열화에 강건한 SLAM — 설계 문서 (v0.3, 포렌식 반영)

> v0.1(앵커 전체 배선)은 과거 실패를 반복할 뻔했고, v0.2(앵커 매장)는 성급했다.
> **삭제된 앵커 코드(cea7cdc0^)와 실패 이력을 포렌식 정밀분석**한 결과, 균형점을
> 찾았다: 앵커는 좁은 역할의 **최소·올바른 느슨결합 데이텀**으로만 부활, CSM이 주권.
...
## 5. 마일스톤
1. **M1 — sim 최소실용**(heading σ tier + rtk_auto 재적합 + psrdiff σ 0.72). 저위험·식별가능.
2. **M2 — GNSS-free 모션 사다리 튜플화(B-모션) + supervisor.** 브리지 rung 대부분 구현됨.
3. **M3 — 퍼셉션/CSM 강건성**(association·CSM 신뢰) — 실패 지배원(포렌식: 상류 50%).
4. **M4 — L3 시나리오 + §4 몬테카를로 하네스.**
5. **M5 — 최소 앵커(부록 A) + A/B 게이트.** CSM 못 이기면 georef/복구 한정.
