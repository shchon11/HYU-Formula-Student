...
def dist_to_p(t_glob: np.ndarray, path: list, p: np.ndarray):
    # scipy versions may return nested arrays when t_glob is shape-(1,).
    # Flatten both vectors to keep euclidean() input strictly 1-D.
    t_val = float(np.atleast_1d(t_glob)[0])
    s = np.asarray(interpolate.splev(t_val, path), dtype=float).reshape(-1)
