...
def calc_vel_profile_brake(kappa: np.ndarray,
    ...
    the first dimension is the waypoint, the second is the velocity and the third contains the acceleration limit
    columns -> DIM = NO_WAYPOINTS_CLOSED x NO_VELOCITY ENTRIES x NO_GGV_COLUMNS"""
    ...
    for i in range(no_points - 1):
        ...
        decel_col = 3 if ggv_mod.shape[1] == 4 else 1
        ggv_mod[:, decel_col] *= -1.0  # use negative acceleration in x axis for forward deceleration
