...
# Headless mode: skip all cv2.imshow()/cv2.waitKey() previews so the script can
# run unattended (e.g. from map_csv_to_icra_global_plan.py). Enable with
# --headless / --no-gui or LANE_GENERATOR_HEADLESS=1.
HEADLESS = ("--headless" in sys.argv) or ("--no-gui" in sys.argv) \
    or os.environ.get("LANE_GENERATOR_HEADLESS", "0") == "1"
...
if __name__ == "__main__":
    ...
    if not show_result([track_img], title="track"):
        exit(0)
