...
Includes global_planner_trajectory_publisher.launch.py (which starts the data
publisher, frenet_odom_node, and wpnt_publisher) and adds the debug visualizer node. All nodes
share a single params file (config/global_planner.yaml); each reads only its own
...
global_planner_trajectory_publisher.launch.py.
"""
...
def generate_launch_description():
    pkg = FindPackageShare("global_planner")

    default_params = PathJoinSubstitution([pkg, "config", "global_planner.yaml"])
    ...
    trajectory_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            PathJoinSubstitution(
                [pkg, "launch", "global_planner_trajectory_publisher.launch.py"]
            )
        ),
        launch_arguments={
    ...
    debug_node = Node(
        ...
        executable="global_planner_debug_visualizer_node",
        name="global_planner_debug_visualizer_node",
        output="screen",
