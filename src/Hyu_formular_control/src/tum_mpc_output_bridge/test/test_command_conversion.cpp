...
TEST(CommandConversion, Converts1300NewtonsToOneMeterPerSecondSquared)
{
  const auto result = BuildCommand(ValidInput(), true, 0.01, ConversionConfig{});
...
TEST(CommandConversion, ClampsAccelerationAndSteering)
{
  ...
  EXPECT_DOUBLE_EQ(positive.command.acceleration_mps2, 5.2);
  EXPECT_DOUBLE_EQ(negative.command.acceleration_mps2, -5.2);
  EXPECT_DOUBLE_EQ(positive.command.steering_angle_rad, 0.2);
  EXPECT_DOUBLE_EQ(negative.command.steering_angle_rad, -0.2);
}
