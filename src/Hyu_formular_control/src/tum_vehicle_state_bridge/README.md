...
```zsh
cd /home/simseunghwan/Formular_control/Hyu_formular_control
source /opt/ros/humble/setup.zsh
source /home/simseunghwan/HYU-Formula-Student/install/setup.zsh
...
`/tmcp/vehicle_state`의 `tmcp` 철자는 오타처럼 보이지만 현재 MPC wrapper의
기본 구독 토픽과 맞추기 위해 의도적으로 유지한다.
...
브리지는 `/tmcp/vehicle_state` 발행을 중단한다. 오래된 조향이나 가속도를 정상값처럼
...
| `output_topic` | `/tmcp/vehicle_state` | MPC vehicle-state 출력 |
...
4. 요청 조향과 적용 조향의 차이를 확인한다.
   ...
   ros2 topic echo /tmcp/vehicle_state
   ```
...
8. 세 입력 중 하나를 끊었을 때 0.2초 후 `/tmcp/vehicle_state`가 멈추고, 다시
   연결하면 자동 복구되는지 확인한다.

   ```zsh
   ros2 topic hz /tmcp/vehicle_state
   ```
