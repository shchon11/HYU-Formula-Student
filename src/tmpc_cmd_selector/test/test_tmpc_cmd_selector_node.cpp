...
TEST_F(TmpcCmdSelectorNodeTest, SelectsCommandsAndLatchesGlobalFault)
{
  ...
  publishInputs("GLOBAL", false);
  ASSERT_TRUE(spinUntilStatus("FAULT_BRAKE", 300ms));
  EXPECT_DOUBLE_EQ(last_output_.drive.speed, 0.0);
  EXPECT_DOUBLE_EQ(last_output_.drive.acceleration, -5.0);
  EXPECT_DOUBLE_EQ(last_output_.drive.steering_angle, 0.0);

  publishInputs("GLOBAL", true);
  spinFor(150ms);
  EXPECT_EQ(last_status_, "FAULT_BRAKE");
