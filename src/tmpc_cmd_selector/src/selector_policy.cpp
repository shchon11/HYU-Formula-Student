...
SelectorDecision SelectorPolicy::update(const SelectorInputs & inputs)
{
  ...
  if (state == PlanningState::kLocal) {
    if (inputs.local_command_valid && fresh(
  ...
  if (fault_latched_) {
    return {CommandSource::kSafeBrake, SelectorStatus::kFaultBrake};
  }
  ...
  if (inputs.local_command_valid && fresh(
      ...
      config_.local_command_timeout_sec))
  {
    return {CommandSource::kPurePursuit, SelectorStatus::kGlobalWaitingTmpc};
  }
  return {CommandSource::kSafeBrake, SelectorStatus::kInputBrake};
}
...
bool SelectorPolicy::faultLatched() const
{
