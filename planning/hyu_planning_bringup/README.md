...
이 구성에서는 Pure Pursuit가 `/control/pp/cmd`, TMPC bridge가 `/control/tmpc/cmd`를
발행하며 `hyu_cmd_selector`만 최종 `/vehicle/cmd`를 발행합니다. GLOBAL 진입 후 TMPC
...
실행해야 합니다. 두 launch를 동시에 실행하면 기존 Pure Pursuit가 `/vehicle/cmd`에 남아
...
ros2 topic echo /control/selector/status
```
...
| `/vehicle/cmd` | Standard launch: `hyu_pure_pursuit_node`; TMPC launch: `hyu_cmd_selector` |
...
`/planning/global_path_waypoints`; `/planning/path` is reserved for the
...
| `controller_cmd_topic` | `/vehicle/cmd` | Pure Pursuit command output. Hybrid TMPC launch sets `/control/pp/cmd`. |
