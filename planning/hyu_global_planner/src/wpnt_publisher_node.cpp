...
: Node("wpnt_publisher", options)
{
  ...
  const std::string default_ggv_path =
    ament_index_cpp::get_package_share_directory("hyu_global_planner") +
  ...
  hyu_global_planner::tmpc::BuilderConfig builder_config;
  ...
  if (tmpc_input_heading_convention == "ros_yaw") {
    builder_config.heading_convention =
      hyu_global_planner::tmpc::HeadingConvention::kRosEastZero;
  } else if (tmpc_input_heading_convention == "formula_north_zero") {
    builder_config.heading_convention =
      hyu_global_planner::tmpc::HeadingConvention::kFormulaNorthZero;
  } else {
  ...
  std::optional<hyu_global_planner::tmpc::GgvTable> ggv_table;
  if (tmpc_ggv_csv_path.empty()) {
    ggv_table = hyu_global_planner::tmpc::GgvTable::fromVectors(
      tmpc_ggv_speed_mps, tmpc_ggv_ax_max_mps2, tmpc_ggv_ay_max_mps2,
      tmpc_ggv_ax_decel_max_mps2, &ggv_error);
  } else {
    ggv_table = hyu_global_planner::tmpc::GgvTable::loadCsv(tmpc_ggv_csv_path, &ggv_error);
  }
  if (!ggv_table) {
    throw std::invalid_argument("Invalid TMPC GGV configuration: " + ggv_error);
  }

  tmpc_builder_ = std::make_unique<hyu_global_planner::tmpc::TmpcTrajectoryBuilder>(
...
void WpntPublisher::onTmpcTimer()
{
  ...
  hyu_global_planner::tmpc::BuildInput build_input;
