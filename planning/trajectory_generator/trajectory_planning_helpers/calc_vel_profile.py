...
def calc_vel_profile(ax_max_machines: np.ndarray,
    ...
    :type m_veh:            float
    :param ggv:             ggv-diagram to be applied: [vx, ax_max, ay_max] or
                            [vx, ax_max, ay_max, ax_decel_max]. Velocity in m/s, accelerations in m/s2.
    ...
    # check shape of ggv
    if ggv is not None and ggv.shape[1] not in (3, 4):
        raise RuntimeError(
            "ggv diagram must consist of [vx, ax_max, ay_max] or "
            "[vx, ax_max, ay_max, ax_decel_max]!")
    ...
    where the first dimension is the waypoint, the second is the velocity and the third contains the acceleration
    limit columns -> DIM = NO_WAYPOINTS_CLOSED x NO_VELOCITY ENTRIES x NO_GGV_COLUMNS"""
...
def calc_ax_poss(vx_start: float,
    ...
    if ggv.ndim != 2 or ggv.shape[1] not in (3, 4):
        raise RuntimeError(
            "ggv must have two dimensions and columns [vx, ax_max, ay_max] or "
            "[vx, ax_max, ay_max, ax_decel_max]!")
    ...
    # 4-column ggv uses ax_decel_max for deceleration; 3-column ggv keeps the old symmetric behavior.
    ax_col = 3 if mode in ['decel_forw', 'decel_backw'] and ggv.shape[1] == 4 else 1
    ax_max_tires = mu * np.interp(vx_start, ggv[:, 0], ggv[:, ax_col])
