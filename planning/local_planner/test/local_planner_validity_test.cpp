...
TEST(LocalPlannerValidity, ValidPathSurvivesUnmatchedRawConeAndOdomCallbacks)
{
  ...
  planner_options.append_parameter_override(
    "waypoints_topic", "/t9_local_planner/waypoints");
  planner_options.append_parameter_override("path_topic", "/t9_local_planner/path");
  planner_options.append_parameter_override(
    "validity_topic", "/t9_local_planner/validity");
  planner_options.append_parameter_override("heartbeat_hz", 50.0);

  auto planner = std::make_shared<LocalPlannerNode>(planner_options);
  auto driver = std::make_shared<rclcpp::Node>("local_planner_validity_driver");
  auto cones_publisher = driver->create_publisher<ConeArray>(
    "/t9_local_planner/cones", rclcpp::SensorDataQoS());
  auto odom_publisher = driver->create_publisher<Odometry>(
    "/t9_local_planner/odom", rclcpp::QoS(rclcpp::KeepLast(20)).reliable());

  Observations observations;
  const auto waypoint_subscription = driver->create_subscription<
    eufs_msgs::msg::WaypointArrayStamped>(
    "/t9_local_planner/waypoints", rclcpp::QoS(rclcpp::KeepLast(10)).reliable(),
  ...
  const auto validity_subscription = driver->create_subscription<std_msgs::msg::Bool>(
    "/t9_local_planner/validity", rclcpp::QoS(rclcpp::KeepLast(10)).reliable(),
...
TEST(LocalPlannerValidity, SingleInvalidBuildRetainsFreshPathUntilInputAgeExpires)
{
  ...
  auto planner = std::make_shared<LocalPlannerNode>(planner_options);
  auto driver = std::make_shared<rclcpp::Node>("local_planner_retention_driver");
  auto cones_publisher = driver->create_publisher<ConeArray>(
