#!/usr/bin/env bash
# arace.sh — AUTONOMOUS full stack in one tmux session.
#
#   arace [track] [extra simfull args...]   # start (default track: small_track)
#   arace stop                              # tear down
#   arace attach                            # re-attach
#
...
case "${1:-start}" in
  stop|kill|down)
    tmux kill-session -t "$SESSION" 2>/dev/null && echo "arace: stopped." || echo "arace: no session."
    exit 0 ;;
  attach|a)
...
if [ ! -f "$WS_SETUP" ]; then
  echo "arace: workspace not built ($WS_SETUP missing). Run 'fsb' first." >&2
  exit 1
fi
tmux has-session -t "$SESSION" 2>/dev/null && { echo "arace: already running — 'arace stop' first, or 'arace attach'."; exit 1; }
...
echo "arace: launching AUTONOMOUS stack on track '$TRACK'…"
...
cat <<EOF
arace: up.  attach → 'arace attach'   |   stop everything → 'arace stop'
